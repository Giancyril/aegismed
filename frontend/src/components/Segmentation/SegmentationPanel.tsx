...
import { BrainCircuit, Eye, EyeOff, Sliders, Cpu, Play } from 'lucide-react';
    ...
    <aside className="w-72 bg-clinical-900 border-l border-clinical-750 flex flex-col h-[calc(100vh-3rem-2rem)] select-none">
      ...
      <div className="p-3 border-b border-clinical-750 bg-clinical-850/40">
        <div className="text-[11px] font-medium text-clinical-300 mb-1 flex items-center justify-between">
          ...
          <span className="font-mono text-indigo-300">{jobStatus.modelName}</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
        <div className="text-[11px] font-semibold text-clinical-400 uppercase tracking-wider mb-2 flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono">{labels.length} Regions</span>
        </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-clinical-400 hover:text-clinical-100 p-1"
                title={label.visible ? 'Hide mask' : 'Show mask'}
              >
                {label.visible ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5 text-clinical-600" />}
              </button>
            </div>

            <div className="flex items-center justify-between text-[11px] font-mono text-clinical-400 mb-2">
              <span>Vol: <span className="text-clinical-200">{label.volumeCm3 ?? 0} cm³</span></span>
              {label.confidence && (
                <span>Conf: <span className="text-emerald-400">{(label.confidence * 100).toFixed(1)}%</span></span>
              )}
            </div>
            ...
            <div className="flex items-center space-x-2 text-[10px] text-clinical-400">
              ...
              <input
                ...
                className="w-full h-1 bg-clinical-750 rounded appearance-none cursor-pointer accent-indigo-500"
              />
